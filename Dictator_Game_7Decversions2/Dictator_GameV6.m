...
file = sprintf('Dictator_Game.dat');
...
% Setting the locations and rectangles for the coins and boxes
% Row 1 
COIN(1).coins = [DG.origin(1)+200 DG.origin(2)-200 DG.origin(1)+400 DG.origin(2)];
COIN(2).coins = [DG.origin(1)-100 DG.origin(2)-200 DG.origin(1)+100 DG.origin(2)];
COIN(3).coins = [DG.origin(1)-400 DG.origin(2)-200 DG.origin(1)-200 DG.origin(2)];
...
%                 Screen('FrameRect', DG.wid, DG.unselRecColour, BOX(i).box, DG.unselRecSize);
        ...
        Screen('TextSize', DG.wid, 40);
         ...
         DrawFormattedText(DG.wid, boxcount1, DG.origin(1)-0.3*DG.wRect(3), DG.origin(2)+0.3*DG.wRect(4), [255 255 255]);
         DrawFormattedText(DG.wid, boxcount2, DG.origin(1)+0.3*DG.wRect(3), DG.origin(2)+0.3*DG.wRect(4), [255 255 255]);
...
%          WaitSecs(2);
        ...
        dataFile = fopen(dataFileName, 'a');
        %fprintf(file, 'boxcount1, boxcount2, RT'); 
        fprintf(dataFile, formatString, boxcount1, boxcount2, RT);
        fclose(dataFile);
...
%         end
    ...
    message = sprintf('You have put %s coins into the other child''s box, and %s into your own!', boxcount1, boxcount2);
