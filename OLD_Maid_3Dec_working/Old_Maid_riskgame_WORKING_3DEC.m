...
% this function is succesful in:
% - mapping animals and an Old Maid card
% - showign stimuli in place of the rectangles (Old Maid in red)
% - doing proper press, e.g. waiting for space press and collecting mouse
% clicks in between.
% - saving a record of how many cards picked, whether it was a loss or not
% and RT.
% still need to add 5 trials and a score counter.
...
formatString = '%s,%d,%d,%d'; % this is for dataoutput
...
sequence = sequence(randperm(length(sequence)));
    ...
    fixation = 'Hello there';
        ...
        RECS(1).rectangles = [OM.origin(1)-800 OM.origin(2)-350 OM.origin(1)-500 OM.origin(2)-15];
        RECS(2).rectangles = [OM.origin(1)-480 OM.origin(2)-350 OM.origin(1)-180 OM.origin(2)-15];
        RECS(3).rectangles = [OM.origin(1)-160 OM.origin(2)-350 OM.origin(1)+160 OM.origin(2)-15];
